...
public class PdfFileDAO {
    public void saveFileInfo(PdfFile pdfFile) throws SQLException {
        String sql = "INSERT INTO pdf_files (file_name, file_path, file_description, file_category) VALUES (?, ?, ?, ?)";
        try (Connection connection = DatabaseConnection.getConnection();
             PreparedStatement statement = connection.prepareStatement(sql)) {
            statement.setString(1, pdfFile.getFileName());
            statement.setString(2, pdfFile.getFilePath());
            statement.setString(3, pdfFile.getFileDescription());
            statement.setString(4, pdfFile.getFileCategory());
    ...
    public List<PdfFile> getAllFiles() throws SQLException {
        ...
        try (Connection connection = DatabaseConnection.getConnection();
            ...
            while (resultSet.next()) {
                ...
                String fileDescription = resultSet.getString("file_description");
                String fileCategory = resultSet.getString("file_category");
                String filePath = resultSet.getString("file_path");
                String uploadedAt = resultSet.getString("uploaded_at");
                files.add(new PdfFile(id, fileName, fileDescription, fileCategory, filePath, uploadedAt));
            }
        }
        return files;
    }
